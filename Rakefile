...
task :gemspec do
  ...
  test_files  = "test/#{ name }.rb" if File.file?("test/#{ name }.rb")
  #summary     = object.respond_to?(:summary) ? object.summary : "summary: #{ name } kicks the ass"
  #description = object.respond_to?(:description) ? object.description : "description: #{ name } kicks the ass"
  #license     = object.respond_to?(:license) ? object.license : "Ruby"
  summary     = object.respond_to?(:summary) ? object.summary : abort("missing #{ object }.summary")
  description = object.respond_to?(:description) ? object.description : abort("missing #{ object }.description")
  license     = test(?e, 'LICENSE') ? IO.binread('LICENSE') : abort("missing LICENSE") 
...
#
  ...
  This.name = File.basename(Dir.pwd).sub(/[-].*$/, '')
  This.basename = File.basename(This.file)

# load _lib
#
  _libpath = ["./lib/#{ This.name }/_lib.rb", "./lib/#{ This.name }.rb"]
  _lib = _libpath.detect{|l| test(?s, l)}
  unless _lib
    abort "could not find a _lib in ./lib/ via #{ _libpath.join(':') }"
  end
  This._lib = _lib
  require This._lib
...
#
  ...
  %w[ summary description dependencies ].each do |method|
    if This.object.respond_to?(method)
      getter = "#{ method }"
      setter = "#{ method }="
      This.send(setter, This.object.send(getter))
    end
  end
